...
## 7) Official manuals, guides, and references (from JIIDE 2025 presentation)

These are **authoritative references published by DGT** and explicitly referenced in the JIIDE 2025 presentation.  
They should be considered the *primary source of truth* for API behavior and supported parameters.

### 7.1 OGC API User Manual (DGT)
**Purpose**
- Explains how to use DGT’s OGC API services (Features, Tiles, Maps, Records)
- Targeted at GIS users, developers, and data analysts

**URL**
- https://dgterritorio.github.io/ogcapi-user/

**Use this when**
- You need clarification on request parameters (`bbox`, `limit`, pagination)
- You want examples aligned with DGT’s actual configuration
- You are unsure which OGC API modules are enabled

---

### 7.2 Interactive OpenAPI Documentation
**Purpose**
- Machine-readable + interactive documentation generated from the API
- Allows you to test endpoints directly in the browser

**URL**
- https://ogcapi.dgterritorio.gov.pt/openapi?f=html

**Use this when**
- You want to quickly inspect available endpoints
- You need to confirm exact path names and parameters
- You want to test `/collections`, `/items`, `/tiles` without writing code

---

### 7.3 OGC API Landing Page (Service Entry Point)
**Purpose**
- Root entry point required by OGC API standards
- Starting point for automated discovery

**URL**
- https://ogcapi.dgterritorio.gov.pt/

**Use this when**
- Building fully standards-compliant clients
- Implementing generic OGC API crawlers or harvesters

---

### 7.4 PoInT Catalogue (OGC API Records)
**Purpose**
- National catalogue for geospatial datasets and services
- Used to discover SRUP datasets (RAN, REN, etc.)
- Provides links to WMS/WFS GetCapabilities
...
**Use this when**
- You need to find authoritative services for legal constraints (SRUP)
- You want to discover datasets not directly exposed as OGC API Features

---

### 7.5 QGIS Integration (experimental, referenced in presentation)
**Purpose**
- Facilitates direct connection from QGIS to DGT’s OGC API

**Notes**
- Mentioned as *experimental* in the JIIDE presentation
- Availability and stability may vary by QGIS version

**Recommended fallback**
- Use native QGIS support for:
  - OGC API – Features
  - WMS / WFS (for SRUP datasets like RAN / REN)

---

### 7.6 Standards referenced in the presentation
These explain *why* the platform behaves the way it does:

- **OGC API – Features**
- **OGC API – Tiles**
- **OGC API – Maps**
- **OGC API – Records**
- **EU Open Data Directive (EU) 2019/1024**
- **EU Regulation 2023/138 (High-Value Datasets)**

These standards justify:
- Open access
- Machine-readability
- Use of APIs instead of file-only distribution

---

### 7.7 Recommended reading order (for LLMs or humans)
1. OGC API User Manual  
2. OpenAPI interactive docs  
3. `/collections?f=json` exploration  
4. CRUS extraction via `/items`  
5. SRUP (RAN / REN) discovery via PoInT → WMS/WFS  

This mirrors the workflow presented at JIIDE 2025.
