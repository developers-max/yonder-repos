...
This module provides unified access to all geographic data layers available for Portugal.

## Overview

| Category | Layer | Source | Access Method | Speed |
|----------|-------|--------|---------------|-------|
| **Administrative** | District | DGT OGC API | `distritos` collection | Fast |
| | Municipality | DGT OGC API | `municipios` collection | Fast |
| | Parish | DGT OGC API | `freguesias` collection | Fast |
| | NUTS3 | DGT OGC API | `nuts3` collection | Fast |
| **Cadastre** | Cadastro Predial | DGT OGC API | `cadastro` collection | Fast |
| | BUPi | ArcGIS REST | Municipal services | Medium |
| **Zoning** | CRUS | DGT OGC API | `crus_<municipio>` collections | Fast |
| | REN | Municipal ArcGIS / DGT WFS | ArcGIS REST / SRUP WFS | Medium |
| | RAN | Municipal ArcGIS / DGT WFS | ArcGIS REST / SRUP WFS | Medium |
| **Land Use** | COS 2018 | DGT WMS | GetFeatureInfo | Fast |
| | CLC 2012 | DGT WMS | GetFeatureInfo | Fast |
| | Built-up Areas | DGT WMS | GetFeatureInfo | Fast |
| **Elevation** | Elevation | Open-Elevation API | REST API | Fast |

---

## Data Sources

### 1. DGT OGC API (Primary)
**Base URL:** `https://ogcapi.dgterritorio.gov.pt`

Modern OGC API Features service providing GeoJSON access to:
- Administrative boundaries (districts, municipalities, parishes, NUTS3)
- Cadastral parcels
...
### 2. DGT WMS Services (Land Use)
WMS GetFeatureInfo for land cover classification:

| Layer | WMS URL | Layer Name |
|-------|---------|------------|
| COS 2018 | `https://geo2.dgterritorio.gov.pt/geoserver/COS2018/wms` | `COS2018_v2` |
| CLC 2012 | `https://geo2.dgterritorio.gov.pt/geoserver/CLC/wms` | `CLC2012` |
| Built-up | `https://geo2.dgterritorio.gov.pt/geoserver/AE/wms` | `areas_edificadas` |

### 3. DGT SRUP WFS (Zoning Restrictions)
...
- ✅ Filter by `CONCELHO` (municipality) for fast queries
...
| REN Norte | `https://servicos.dgterritorio.pt/SDISNITWFSSRUP_REN_NORTE/WFService.aspx` | `gmgml:REN_Norte` |
| REN Centro | `https://servicos.dgterritorio.pt/SDISNITWFSSRUP_REN_CENTRO/WFService.aspx` | `gmgml:REN_Centro` |
| REN LVT | `https://servicos.dgterritorio.pt/SDISNITWFSSRUP_REN_LVT/WFService.aspx` | `gmgml:REN_LVT` |
| REN Alentejo | `https://servicos.dgterritorio.pt/SDISNITWFSSRUP_REN_ALENTEJO/WFService.aspx` | `gmgml:REN_Alentejo` |
| REN Algarve | `https://servicos.dgterritorio.pt/SDISNITWFSSRUP_REN_ALGARVE/WFService.aspx` | `gmgml:REN_Algarve` |
